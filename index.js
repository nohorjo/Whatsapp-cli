...
const printQRcode = async page => {
    ...
    const tryPrint = async () => {
        try {
            ...
            await page.$(SEL_QR).then(x => x.screenshot({ path: qrFile }));
            log('Processing code');
            ...
            qrDecoder.decode(await jimp.read(fs.readFileSync(qrFile)).then(x => x.bitmap));
        } catch (e) {
    ...
    try {
        ...
        const printPeople = async () => {
            for (let i = 0; i < people.length; i++) {
                log(`${i + 1} - ${await people[i].getProperty('textContent').then(x => x.jsonValue())}`);
            }
        };
        ...
        readAnswer(async (answer) => {
            ...
            const printMessage = async msg => {
                ...
                if (theMessage) {
                    const msgText = await theMessage.getProperty('textContent').then(x => x.jsonValue());
                    if (outMsg) {
                        messageOut(`> ${msgText}`);
                    } else {
            ...
            messageScanner = setInterval(async () => {
                ...
                if (msg) {
                    const msgContent = await  msg.getProperty('textContent').then(x => x.jsonValue());
            ...
            const readInput = () => readAnswer(async line => {
                if (line.trim()) {
                    await page.$(SEL_MSG_INPUT).then(x => x.type(` ${line}`));
